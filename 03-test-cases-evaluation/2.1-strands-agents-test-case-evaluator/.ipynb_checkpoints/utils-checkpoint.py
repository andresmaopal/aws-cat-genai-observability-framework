...
class UnifiedTester:
    ...
    def run_test(self, 
                 ...
                 tool: List = None, 
                 trace: Any = None,
                 trace_attributes: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """
    ...
    def list_models_by_provider(self):
        ...
        print(f"\n📋 AVAILABLE MODELS ({len(self.models)} total)")
        print("=" * 100)
        ...
        for provider in sorted(provider_groups.keys()):
            ...
            for model in models:
                ...
                print(f"  {region_badge} {model['name']:<25} | {model['region']:<12} | {tooling_badge}")
        
        print(f"\n📊 Summary by Provider:")
        for provider in sorted(provider_groups.keys()):
            print(f"  • {provider}: {len(provider_groups[provider])} models")
        print("=" * 100)
...
<analysis>
[Your one paragraph detailed analysis of whether the response meets the criteria in SPANISH language]
</analysis>
